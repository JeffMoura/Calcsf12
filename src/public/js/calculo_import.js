...
//CÓDIGO JAVASCRIPT QUE RECEBE OS DADOS DO FORMULÁRIO DA PÁGINA DE IMPORTAÇÃO
$(function () {
    ...
    var CALC = {
        ...
        //função que retornar o índice da linha
        ...
        botao_excluir: function (event) {
            ...
            console.log(CALC.removeLinha(targ))
      ...
      //      CALC.calcDescritivos();
         ...
         /*=====================================  FUNÇÃO ADICIONAR NOVAS LINHAS (ITENS) ===================================================*/
        ...
        adicionar_linha: function () {
            ...
            var input;
            //incrementa a variável linhaCont
            linhaCont++;
            //recebe a classe item_linha que é o ID da linha
            linha.className = 'item_linha';
            //a variável recebe o id da classe item_linha e adiciona o incrementador
            linha.id = 'item_linha' + linhaCont;

            //entrada de um novo campo id, que exibe o número da linha automaticamente no campo
            ...
            input = $('<input class="sfum form-control" type="text" onkeypress="return onlynumberOP03();" maxlength="1" id="ssfum' + linhaCont + '" name="ssfum[]" >');
          //  input.keyup(CALC.onkeyup).attr("autocomplete", "off"); //Chama a função para calcular quando digitar os valores nos inputs, e desabilita a função autocompletar de campos do formulário
            ...
            $(linha.insertCell()).append(input);
            input = $('<input class="sfdois form-control" type="text" onkeypress="return onlynumberOP02();" maxlength="1" id="ssfdois' + linhaCont + '" name="ssfdois[]">');
            //input.keyup(CALC.onkeyup).attr("autocomplete", "off");
            //entrada de um novo campo SF3
            $(linha.insertCell()).append(input);
            input = $('<input class="sftres form-control" type="text" onkeypress="return onlynumberOP02();" maxlength="1" id="ssftres' + linhaCont + '" name="ssftres[]">');
           // input.keyup(CALC.onkeyup).attr("autocomplete", "off");
            //entrada de um novo campo SF4
            $(linha.insertCell()).append(input);
            input = $('<input class="sfquatro form-control" type="text" onkeypress="return onlynumberOP01();"  maxlength="1" id="ssfquatro' + linhaCont + '" name="ssfquatro[]">');
            //input.keyup(CALC.onkeyup).attr("autocomplete", "off");
            //entrada de um novo campo SF5
            $(linha.insertCell()).append(input);
            input = $('<input class="sfcinco form-control" type="text" onkeypress="return onlynumberOP01();" maxlength="1" id="ssfcinco' + linhaCont + '" name="ssfcinco[]">');
            //input.keyup(CALC.onkeyup).attr("autocomplete", "off");
            //entrada de um novo campo SF6
            $(linha.insertCell()).append(input);
            input = $('<input class="sfseis form-control"   type="text" onkeypress="return onlynumberOP01();" maxlength="1"  id="ssfseis' + linhaCont + '" name="ssfseis[]">');
            //input.keyup(CALC.onkeyup).attr("autocomplete", "off");
            //entrada de um novo campo SF7
            $(linha.insertCell()).append(input);
            input = $('<input class="sfsete form-control"   type="text" onkeypress="return onlynumberOP01();" maxlength="1" id="ssfsete' + linhaCont + '" name="ssfsete[]">');
            //input.keyup(CALC.onkeyup).attr("autocomplete", "off");
            //entrada de um novo campo SF8
            $(linha.insertCell()).append(input);
            input = $('<input class="sfoito form-control"   type="text" onkeypress="return onlynumberOP03();" maxlength="1" id="ssfoito' + linhaCont + '" name="ssfoito[]">');
            //input.keyup(CALC.onkeyup).attr("autocomplete", "off");
            //entrada de um novo campo SF9
            $(linha.insertCell()).append(input);
            input = $('<input class="sfnove form-control"   type="text" onkeypress="return onlynumberOP04();" maxlength="1" id="ssfnove' + linhaCont + '" name="ssfnove[]">');
            //input.keyup(CALC.onkeyup).attr("autocomplete", "off");
            //entrada de um novo campo SF10
            $(linha.insertCell()).append(input);
            input = $('<input class="sfdez form-control"   type="text" onkeypress="return onlynumberOP04();" maxlength="1" id="ssfdez' + linhaCont + '" name="ssfdez[]">');
            //input.keyup(CALC.onkeyup).attr("autocomplete", "off");
            //entrada de um novo campo SF11
            $(linha.insertCell()).append(input);
            input = $('<input class="sfonze form-control"   type="text" onkeypress="return onlynumberOP04();" maxlength="1" id="ssfonze' + linhaCont + '" name="ssfonze[]">');
            //input.keyup(CALC.onkeyup).attr("autocomplete", "off");
            //entrada de um novo campo SF12
            $(linha.insertCell()).append(input);
            input = $('<input class="sfdoze form-control"   type="text" onkeypress="return onlynumberOP03();" maxlength="1" id="ssfdoze' + linhaCont + '" name="ssfdoze[]">');
            //input.keyup(CALC.onkeyup).attr("autocomplete", "off");
            //Calculo do PCS
            $(linha.insertCell()).append(input);
            input = $('<input class="ppcs form-control"  readonly="readonly" type="text" id="pcs' + linhaCont + '" name="pcs[]">');
            //Calculo do MCS
            $(linha.insertCell()).append(input);
            input = $('<input class="mmcs form-control"  readonly="readonly"  type="text" id="mcs' + linhaCont + '" name="mcs[]">');

            //entrada de um novo campo botão de excluir
            $(linha.insertCell()).append(input);
            input = $('<input type="button"  readonly="readonly" class="btn btn-danger remover_botao" name="deletar_linha" value="Excluir" id="deletar_linha">');
            input.click(CALC.botao_excluir);
            $(linha.insertCell()).append(input);
            $(linha).children('input[type="text"]').attr("autocomplete", "off");

            /*A cada inserção de linha, chama as duas funções abaixo para atualizar os calculos e seguir a regra de calcular apenas
            se todos os campos tiverem preenchidos*/
            CALC.calcRelatorios();
            CALC.calcDescritivos();
        },

         /*=====================================  FUNÇÃO RENUMERAR AS LINHAS  ===================================================*/
        ...
        renumerarLinhas: function () {
         ...
         /*=====================================  FUNÇÃO LIMPAR TODOS OS CAMPOS (RESETAR) ===================================================*/
        ...
        /* ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
        |||||||||||||||||||||||||||||  FUNÇÃO CALCULAR LINHAS (PCS e MCS) DOS DADOS IMPORTADOS  ||||||||||||||||||||||||||||||||||||||
        ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||*/

        calcula_linha_import: function () {
            
            //variável recebe as linhas da lista da tabela
            var trs = document.querySelectorAll('#itemlist tr');
    
        
            trs.forEach((linha) => {              //executa o cálculo para cada linha da tabela (calculadora)
                    
                
                        //variáveis irão receber os valores dos campos
                        var sf1 = linha.querySelector('input[id=ssfum1]');
                        var sf2 = linha.querySelector('input[id=ssfdois1]');
                        var sf3 = linha.querySelector('input[id=ssftres1]');
                        var sf4 = linha.querySelector('input[id=ssfquatro1]');
                        var sf5 = linha.querySelector('input[id=ssfcinco1]');
                        var sf6 = linha.querySelector('input[id=ssfseis1]');
                        var sf7 = linha.querySelector('input[id=ssfsete1]');
                        var sf8 = linha.querySelector('input[id=ssfoito1]');
                        var sf9 = linha.querySelector('input[id=ssfnove1]');
                        var sf10 = linha.querySelector('input[id=ssfdez1]');
                        var sf11 = linha.querySelector('input[id=ssfonze1]');
                        var sf12 = linha.querySelector('input[id=ssfdoze1]');
                        var pcs = linha.querySelector('input[id=pcs1]');
                        var mcs = linha.querySelector('input[id=mcs1]');
                        ...
                        if ((sf1.value === '') || (sf2.value === '') || (sf3.value === '') || (sf3.value === '') || (sf4.value === '') || (sf5.value === '') || (sf6.value === '') || (sf7.value === '')
        ...
        /* FUNÇÃO IRÁ CALCULAR A MÉDIA GERAL, DESVIO PADRÃO, COEFICIÊNCIA DE VARIAÇÃO E DEMAIS ANÁLISES DESCRITIVAS DO PCS E MCS*/
        ...
        calcDescritivos: function () {
            ...
            //variáveis que vão receber os ID dos campos de relatório para repassar o resultado
            ...
            trs.forEach((linhas) => {       
                ...
                listaPCS = parseFloat(linhas.querySelector('input[id=pcs1]').value);
                listaMCS = parseFloat(linhas.querySelector('input[id=mcs1]').value);
		...
		calcRelatorios: function () {
			...
			//variáveis que vão receber os nomes dos campos de relatório Geral (DOMÍNIOS) para repassar o resultado
		
            REL_PCS_MEDIA = document.querySelector('input[id=rel_pcs_media]');
			REL_MCS_MEDIA = document.querySelector('input[id=rel_mcs_media]');
			REL_PCS_DESVIOP = document.querySelector('input[id=rel_pcs_desviopadrao]');
			REL_MCS_DESVIOP = document.querySelector('input[id=rel_mcs_desviopadrao]');
			REL_PCS_COEFICIENTEV = document.querySelector('input[id=rel_pcs_coeficiente]');
			REL_MCS_COEFICIENTEV = document.querySelector('input[id=rel_mcs_coeficiente]');
			REL_PCS_MAX = document.querySelector('input[id=rel_pcs_valor_maximo]');
			REL_MCS_MAX = document.querySelector('input[id=rel_mcs_valor_maximo]');
			REL_PCS_MIN = document.querySelector('input[id=rel_pcs_valor_minimo]');
			REL_MCS_MIN = document.querySelector('input[id=rel_mcs_valor_minimo]');
			REL_PCS_AMPLITUDE = document.querySelector('input[id=rel_pcs_amplitude]');
			REL_MCS_AMPLITUDE = document.querySelector('input[id=rel_mcs_amplitude]');

			//variáveis que vão receber os nomes dos campos de Médias (SF) para repassar o resultado
			MEDIASF1 = document.querySelector('input[id=mediaSF1]');
			MEDIASF2 = document.querySelector('input[id=mediaSF2]');
			MEDIASF3 = document.querySelector('input[id=mediaSF3]');
			MEDIASF4 = document.querySelector('input[id=mediaSF4]');
			MEDIASF5 = document.querySelector('input[id=mediaSF5]');
			MEDIASF6 = document.querySelector('input[id=mediaSF6]');
			MEDIASF7 = document.querySelector('input[id=mediaSF7]');
			MEDIASF8 = document.querySelector('input[id=mediaSF8]');
			MEDIASF9 = document.querySelector('input[id=mediaSF9]');
			MEDIASF10 = document.querySelector('input[id=mediaSF10]');
			MEDIASF11 = document.querySelector('input[id=mediaSF11]');
			MEDIASF12 = document.querySelector('input[id=mediaSF12]');


			//variáveis que vão receber os nomes dos campos de Desvio Padrão (SF) para repassar o resultado
			DESVIOPSF1 = document.querySelector('input[id=desviopadraoSF1]');
			DESVIOPSF2 = document.querySelector('input[id=desviopadraoSF2]');
			DESVIOPSF3 = document.querySelector('input[id=desviopadraoSF3]');
			DESVIOPSF4 = document.querySelector('input[id=desviopadraoSF4]');
			DESVIOPSF5 = document.querySelector('input[id=desviopadraoSF5]');
			DESVIOPSF6 = document.querySelector('input[id=desviopadraoSF6]');
			DESVIOPSF7 = document.querySelector('input[id=desviopadraoSF7]');
			DESVIOPSF8 = document.querySelector('input[id=desviopadraoSF8]');
			DESVIOPSF9 = document.querySelector('input[id=desviopadraoSF9]');
			DESVIOPSF10 = document.querySelector('input[id=desviopadraoSF10]');
			DESVIOPSF11 = document.querySelector('input[id=desviopadraoSF11]');
			DESVIOPSF12 = document.querySelector('input[id=desviopadraoSF12]');

			//variáveis que vão receber os nomes dos campos de Coeficiente (SF) para repassar o resultado
			COEFICIENTEVSF1 = document.querySelector('input[id=coeficienteSF1]');
			COEFICIENTEVSF2 = document.querySelector('input[id=coeficienteSF2]');
			COEFICIENTEVSF3 = document.querySelector('input[id=coeficienteSF3]');
			COEFICIENTEVSF4 = document.querySelector('input[id=coeficienteSF4]');
			COEFICIENTEVSF5 = document.querySelector('input[id=coeficienteSF5]');
			COEFICIENTEVSF6 = document.querySelector('input[id=coeficienteSF6]');
			COEFICIENTEVSF7 = document.querySelector('input[id=coeficienteSF7]');
			COEFICIENTEVSF8 = document.querySelector('input[id=coeficienteSF8]');
			COEFICIENTEVSF9 = document.querySelector('input[id=coeficienteSF9]');
			COEFICIENTEVSF10 = document.querySelector('input[id=coeficienteSF10]');
			COEFICIENTEVSF11 = document.querySelector('input[id=coeficienteSF11]');
			COEFICIENTEVSF12 = document.querySelector('input[id=coeficienteSF12]');

			//variáveis que vão receber os nomes dos campos de Valor Máximo (SF) para repassar o resultado
			REL_SF1_MAX = document.querySelector('input[id=valor_maximoSF1]');
			REL_SF2_MAX = document.querySelector('input[id=valor_maximoSF2]');
			REL_SF3_MAX = document.querySelector('input[id=valor_maximoSF3]');
			REL_SF4_MAX = document.querySelector('input[id=valor_maximoSF4]');
			REL_SF5_MAX = document.querySelector('input[id=valor_maximoSF5]');
			REL_SF6_MAX = document.querySelector('input[id=valor_maximoSF6]');
			REL_SF7_MAX = document.querySelector('input[id=valor_maximoSF7]');
			REL_SF8_MAX = document.querySelector('input[id=valor_maximoSF8]');
			REL_SF9_MAX = document.querySelector('input[id=valor_maximoSF9]');
			REL_SF10_MAX = document.querySelector('input[id=valor_maximoSF10]');
			REL_SF11_MAX = document.querySelector('input[id=valor_maximoSF11]');
			REL_SF12_MAX = document.querySelector('input[id=valor_maximoSF12]');

			//variáveis que vão receber os nomes dos campos de Valor Mínimo (SF) para repassar o resultado
			REL_SF1_MIN = document.querySelector('input[id=valor_minimoSF1]');
			REL_SF2_MIN = document.querySelector('input[id=valor_minimoSF2]');
			REL_SF3_MIN = document.querySelector('input[id=valor_minimoSF3]');
			REL_SF4_MIN = document.querySelector('input[id=valor_minimoSF4]');
			REL_SF5_MIN = document.querySelector('input[id=valor_minimoSF5]');
			REL_SF6_MIN = document.querySelector('input[id=valor_minimoSF6]');
			REL_SF7_MIN = document.querySelector('input[id=valor_minimoSF7]');
			REL_SF8_MIN = document.querySelector('input[id=valor_minimoSF8]');
			REL_SF9_MIN = document.querySelector('input[id=valor_minimoSF9]');
			REL_SF10_MIN = document.querySelector('input[id=valor_minimoSF10]');
			REL_SF11_MIN = document.querySelector('input[id=valor_minimoSF11]');
			REL_SF12_MIN = document.querySelector('input[id=valor_minimoSF12]');

			//variáveis que vão receber os nomes dos campos de Amplitude (SF) para repassar o resultado
			REL_SF1_AMPLITUDE = document.querySelector('input[id=amplitudeSF1]');
			REL_SF2_AMPLITUDE = document.querySelector('input[id=amplitudeSF2]');
			REL_SF3_AMPLITUDE = document.querySelector('input[id=amplitudeSF3]');
			REL_SF4_AMPLITUDE = document.querySelector('input[id=amplitudeSF4]');
			REL_SF5_AMPLITUDE = document.querySelector('input[id=amplitudeSF5]');
			REL_SF6_AMPLITUDE = document.querySelector('input[id=amplitudeSF6]');
			REL_SF7_AMPLITUDE = document.querySelector('input[id=amplitudeSF7]');
			REL_SF8_AMPLITUDE = document.querySelector('input[id=amplitudeSF8]');
			REL_SF9_AMPLITUDE = document.querySelector('input[id=amplitudeSF9]');
			REL_SF10_AMPLITUDE = document.querySelector('input[id=amplitudeSF10]');
			REL_SF11_AMPLITUDE = document.querySelector('input[id=amplitudeSF11]');
			REL_SF12_AMPLITUDE = document.querySelector('input[id=amplitudeSF12]');
           ...
           trs.forEach((linhas) => {       
              ...
              listaMCS = parseFloat(linhas.querySelector('input[id=mcs1]').value);
				...
				listaSF1 = parseFloat(linhas.querySelector('input[id=ssfum1]' ).value);
				listaSF2 = parseFloat(linhas.querySelector('input[id=ssfdois1]' ).value);
				listaSF3 = parseFloat(linhas.querySelector('input[id=ssftres1]' ).value);
				listaSF4 = parseFloat(linhas.querySelector('input[id=ssfquatro1]' ).value);
				listaSF5 = parseFloat(linhas.querySelector('input[id=ssfcinco1]' ).value);
				listaSF6 = parseFloat(linhas.querySelector('input[id=ssfseis1]' ).value);
				listaSF7 = parseFloat(linhas.querySelector('input[id=ssfsete1]' ).value);
				listaSF8 = parseFloat(linhas.querySelector('input[id=ssfoito1]' ).value);
				listaSF9 = parseFloat(linhas.querySelector('input[id=ssfnove1]' ).value);
				listaSF10 = parseFloat(linhas.querySelector('input[id=ssfdez1]' ).value);
				listaSF11 = parseFloat(linhas.querySelector('input[id=ssfonze1]' ).value);
				listaSF12 = parseFloat(linhas.querySelector('input[id=ssfdoze1]' ).value);
            ...
            //Verifica: Se a média não retorna NaN, é atribuído os valores das médias nos respectivos campos, senão, os inputs ficarão vazios 
            if (!isNaN(media_pcs)) {
    ...
    $('.remover_botao').click(CALC.botao_excluir); //chama a função deletar linha quando clica no botão excluir
    $('#btn_limpar').click(CALC.limpar_campos); //chama a função para reseter o formulário quando clica no botão limpar
    $('#calcular').click(CALC.calcula_linha_import); //chama a função para efetuar os cálculos quando clica no botão CALCULAR
